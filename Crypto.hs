...
phi :: Int -> Int
phi m
  = length [num | num <- [1..m], gcd m num == 1]
...
add :: Char -> Char -> Char
add char1 char2
  = toChar ((toInt char1 + toInt char2) `mod` 26)

-- "substracts" two letters
substract :: Char -> Char -> Char
substract char1 char2
  = toChar ((toInt char1 - toInt char2) `mod` 26)
...
cbcEncrypt k iv x
  ...
  where
    ...
    cbcEncrypt' k [] prevChar
        = []
    cbcEncrypt' k x prevChar
        = ci : cbcEncrypt' k xs ci
        where
...
cbcDecrypt k iv c
  ...
  where 
    ...
    cbcDecrypt' k [] prevChar 
        = []
    cbcDecrypt' k c prevChar
        = xi : cbcDecrypt' k cs ci  
        where 
