...
public class TestSuite {
  ...
  // Adding a vector to a point should not mutate the original point.
  @Test
  public void pointAddDoesNotMutate() {
    Point point = new Point(10, 10);
    point.add(new Point(1, 1));
    assertTrue(equalPoints(point, new Point(10, 10)));
  }

  // A point should not be to the right of itself.
  @Test
  public void pointIsRightOfIsNotReflexive() {
    Point point = new Point(10, 10);
    assertFalse(point.isAbove(point));
  }

  // A point should not be above itself.
  @Test
  public void pointIsAboveOfIsNotReflexive() {
    Point point = new Point(10, 10);
    assertFalse(point.isAbove(point));
  }

  // A point should not be below itself.
  @Test
  public void pointIsBelowIsNotReflexive() {
    Point point = new Point(10, 10);
    assertFalse(point.isBelow(point));
  }
  ...
  // Rectangles representing points and lines should have no area.
  @Test
  public void rectangleAreaArealess() {
    assertEquals(0, new Rectangle(0, 0).area()); // Point
    assertEquals(0, new Rectangle(0, 10).area()); // Vertical line
    assertEquals(0, new Rectangle(10, 0).area()); // Horizontal line
  }

  // Rectangle intersection should be commutative.
  @Test
  public void rectangleIntersectIsCommutative() {
    Rectangle r1 = new Rectangle(new Point(3, 3), new Point(6, 6));
    Rectangle r2 = new Rectangle(new Point(4, 4), new Point(7, 7));
    assertTrue(r1.intersects(r2));
    assertTrue(r2.intersects(r1));
  }

  // Intersection with an arealess rectangle, i.e. a point.
  @Test
  public void rectangleIntersectPoint() {
    Rectangle point = new Rectangle(new Point(3, 3), new Point(3, 3));
    Rectangle rectangle = new Rectangle(new Point(2, 2), new Point(4, 4));
    assertTrue(rectangle.intersects(point));
    assertTrue(point.intersects(rectangle));
  }

  // Rectangles that touch on an edge intersect.
  @Test
  public void rectangleIntersectTouchingEdge() {
    Rectangle r1 = new Rectangle(new Point(5, 5), 10, 10);
    Rectangle r2 = new Rectangle(new Point(15, 5), 10, 10);
    assertTrue(r1.intersects(r2));
    assertTrue(r2.intersects(r1));
  }

  // Rectangles that touch at a vertex intersect.
  @Test
  public void rectangleIntersectTouchingVertex() {
    Rectangle r1 = new Rectangle(new Point(5, 5), 10, 10);
    Rectangle r2 = new Rectangle(new Point(15, 15), 10, 10);
    assertTrue(r1.intersects(r2));
    assertTrue(r2.intersects(r1));
  }
  ...
  // Translating by the zero vector should leave the rectangles in the same position.
  @Test
  public void listTranslateZero() {
    RECTANGLE_LISTS.forEach(l ->
        assertTrue(equalRectangleLists(l, ListAlgorithms.translate(l, new Point(0, 0)))));
  }

  // Scaling by a factor of 1 should leave the rectangles unchanged.
  @Test
  public void listScaleByOne() {
    RECTANGLE_LISTS.forEach(l ->
        assertTrue(equalRectangleLists(l, ListAlgorithms.scale(l, 1))));
  }

  // Scaling by a factor of 0 should squish all rectangles into a point.
  @Test
  public void listScaleByZero() {
    List<Rectangle> scaled = ListAlgorithms.scale(VARIOUS_RECTANGLES, 0);
    assertEquals(0, ListAlgorithms.getSumOfAreas(scaled));
    assertTrue(scaled.stream().mapToInt(Rectangle::getWidth).allMatch(width -> width == 0));
    assertTrue(scaled.stream().mapToInt(Rectangle::getHeight).allMatch(height -> height == 0));
  }

  // Largest area of an empty list of rectangles should be 0.
  @Test
  public void listLargestAreaOfEmptyList() {
    assertEquals(0, ListAlgorithms.findLargestArea(new ArrayList<>()));
  }

  // Maximum height of an empty list of rectangles should be 0.
  @Test
  public void listMaxHeightOfEmptyList() {
  ...
  // Translating by the zero vector should leave the rectangles in the same position.
  @Test
  public void streamTranslateZero() {
    RECTANGLE_LISTS.forEach(l ->
        assertTrue(equalRectangleLists(
            l, StreamAlgorithms.translate(l.stream(), new Point(0, 0)).collect(toList()))));
  }

  // Scaling by a factor of 1 should leave the rectangles unchanged.
  @Test
  public void streamScaleByOne() {
    RECTANGLE_LISTS.forEach(l ->
        assertTrue(equalRectangleLists(
            l, StreamAlgorithms.scale(l.stream(), 1).collect(toList()))));
  }

  // Scaling by a factor of 0 should squish all rectangles into a point.
  @Test
  public void streamScaleByZero() {
    List<Rectangle> scaled =
        StreamAlgorithms.scale(VARIOUS_RECTANGLES.stream(), 0).toList();
    assertEquals(0, StreamAlgorithms.getSumOfAreas(scaled.stream()));
    assertTrue(scaled.stream().mapToInt(Rectangle::getWidth).allMatch(width -> width == 0));
    assertTrue(scaled.stream().mapToInt(Rectangle::getHeight).allMatch(height -> height == 0));
  }

  // Largest area of an empty stream of rectangles should be 0.
  @Test
  public void streamLargestAreaOfEmptyStream() {
    assertEquals(0, StreamAlgorithms.findLargestArea(Stream.empty()));
  }

  // Maximum height of an empty stream of rectangles should be 0.
  @Test
  public void streamMaxHeightOfEmptyStream() {
